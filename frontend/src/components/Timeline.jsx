  ...
  if (events.length === 0) {
    return (
      <Text c="dimmed" ta="center" py="xl">
        No timeline events to display yet.
      </Text>
    );
  }
      ...
      {events.map((e, i) => {
        ...
        return (
          <MTimeline.Item
            ...
            color={
              e.suspicious
                ? SEVERITY_COLORS[e.severity] || 'red'
                : CATEGORY_COLORS[e.category] || 'gray'
            }
            title={
              <Group gap="xs">
                ...
                {e.suspicious && (
                  <Badge size="xs" color={SEVERITY_COLORS[e.severity] || 'red'}>
                    {e.severity}
                  </Badge>
                )}
              </Group>
